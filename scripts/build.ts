...
async function writeBuildInfo(): Promise<void> {
  const info = {
    ...
    git_worktree: runCommand(["git", "status", "--porcelain"]) ? "dirty" : "clean",
  ...
  const contents = Object.entries(info).map(([key, value]) => `${key}=${value}\n`).join("");
...
runCommand([
  process.execPath, "build", "src/cli.ts", "--compile",
  `--target=${target}`, "--outfile", buildPath,
]);
verifyBinary();
await copyFile(buildPath, releasePath);
await writeFile(`${releasePath}.sha256`, `${await hashFile(releasePath)}  ${binaryName}\n`);
