...
export function renderPage(
	...
	const cards = visible
		.map((comparison) => {
			...
			return `<article id="${anchor}" data-group="${group}"><h2><a href="#${anchor}">C${String(index + 1).padStart(3, "0")}</a> · ${escapeHtml(item.title)} <span class="badge">${state}</span></h2>${reasons.length ? `<p class="meta">変更点: ${reasons.join(" / ")}。背景付きの行が表示の差分です。</p>` : ""}<div class="pair">${comparison.previous ? screen(comparison.previous, "変更前（基準）", left) : "<section><h3>変更前</h3><p>基準にないケースです。</p></section>"}${comparison.current ? screen(comparison.current, "変更後（今回）", right) : "<section><h3>変更後</h3><p>今回の収録に含まれていません。</p></section>"}</div></article>`;
		})
		.join("");
