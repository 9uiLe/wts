...
| `src/commands/discard.ts` | 指定セッションの削除計画、保護条件と確認後の再検査、worktree と所属ブランチの破棄 |
...
discard は指定された一つの wts セッションを、マージ状況によらず破棄する。呼び出し元プロジェクトの管理範囲、登録された worktree、セッション記録から対象を決め、所属するルート・スタックブランチを表示して確認する。メイン・実行中・管理範囲外・未管理・ロック中の worktree、ベースブランチや別 worktree で使用中の所属ブランチは保護する。未コミット・未追跡・無視されたファイルを削除するには `--force` が必要であり、確認省略の `--yes` とは独立する。dry-run はファイルの変更状態を含む計画を表示するだけである。確認後に対象とブランチを再検査し、worktree の削除が成功してから所属ブランチを削除する。途中で失敗した場合は失敗を報告し、完了とは扱わない。既定ではリモートへの通信と変更は行わない。`--remote <名前>` 指定時は登録された単一の push 先から同名の所属ブランチと OID を取得し、削除計画に含める。確認後に再検査し、明示的な lease と atomic push でリモートを先に削除する。リモート操作の失敗時はローカルを残し、リモート成功後のローカル失敗は完了範囲を報告する。
