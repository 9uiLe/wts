...
カタログは CLI をシナリオどおりに実行し、入力前の画面と最終画面を収録する。シナリオは実行条件、入力、期待する終了コードを持ち、入力待ちと終了結果を検査する。Git は一時リポジトリとローカル bare origin を使用し、外部サービスと障害はテスト用コマンドで再現する。

収録した ANSI 出力は、カーソル移動、消去、色、日本語の文字幅を反映して HTML に描画する。比較では一時パス、生成名、Git OID などの可変値を正規化し、画面、入力、コマンド、終了コードを照合する。進捗の描画回数は比較対象に含めない。

レビュー画面は対象件数、表示切替と絞り込み、各ケースの出力の順に配置する。選択中の操作と変更状態は文字と配色で識別し、端末出力と画面の操作部を区別する。変更がない状態と絞り込みに一致しない状態を分け、それぞれ全件表示と絞り込み解除を案内する。

基準版は Git 管理の収録 JSON とし、明示的な更新操作で保存する。ケースのタイトルを対応キーとして、全件一覧と変更ケースの一覧を生成する。各 HTML に全ケースの画面を内包し、ローカルファイル間の移動を使わず全件と変更のみを切り替える。収録データと比較結果の JSON、収録範囲の Markdown は開発・調査用に出力する。生成失敗は診断ページで示し、未実測の条件は収録範囲に明記する。生成物とレビュー手順は [開発資料](development.md#端末-ui-の一覧と変更確認)に定義する。
