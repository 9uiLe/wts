...
基準版は Git 管理の収録 JSON とし、明示的な更新操作で保存する。ケースのタイトルを対応キーとして、全件一覧と変更ケースの一覧を生成する。生成失敗は診断ページで示し、未実測の条件は収録範囲に明記する。生成物とレビュー手順は [開発資料](development.md#端末-ui-の一覧と変更確認)に定義する。
