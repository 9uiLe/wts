...
導入には Web とローカル成果物の二つの入口を設ける。どちらも Apple Silicon macOS 上で成果物を検証し、既定では `~/.local/bin/wts` に配置する。シェルの設定ファイル、認証、Gatekeeper の設定は変更しない。

| 入口 | 入力と配置条件 |
| --- | --- |
| `scripts/web-install.sh` | Pages の配布対象タグまたは `--version` で同じ Release のバイナリ・SHA-256・`BUILD_INFO` を選択し、SHA-256 とビルド情報のバージョン・ターゲットを検証する。`--install-dir` で配置先を指定できる |
| `scripts/install.sh` | 取得済みの成果物ディレクトリを受け取り、3 ファイルの存在と SHA-256 を検証する。`--with-deps` 指定時は Homebrew による Git・gh の導入成功を配置条件とする |

Web インストーラーは単独で実行でき、リポジトリの clone や別のスクリプトを要求しない。取得・検証・配置が失敗した場合は既存の wts を保持する。配置先の wts がシンボリックリンクや通常ファイル以外の場合は置き換えない。

Web インストールの完了時には、配置先と `wts --version`、`wts --help`、`wts doctor --check` を表示する。配置先が PATH にない場合は、zsh の設定ファイルへ配置先を登録するコマンドと、その場で読み込むコマンドを先に表示する。既定以外の配置先も表示に反映し、シェルの特殊文字を含むパスを安全に渡す。PATH に含まれる場合は設定案内を省く。

Git・gh の手動導入、Homebrew、任意の命名環境、認証は実行環境側で用意する。インストール後の依存・認証確認は `doctor --check` が担う。
