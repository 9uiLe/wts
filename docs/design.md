...
worktree 名からルートブランチを推測しない。stack・restack・discard はセッション識別情報と設定の管理範囲を検査する。cleanup は独自に PR とローカル変更を調査し、セッション識別情報の存在だけを削除根拠にはしない。
...
| `src/commands/discard.ts` | セッションの削除計画と確認、リモート・worktree・ローカルブランチの削除、完了範囲の報告 |
...
discard は一つのセッションを明示的に破棄する。削除対象と実行順序は「[セッションの破棄](#セッションの破棄)」で定義する。
...
`discard <path>` は、worktree のルートパスで指定された一つのセッションを、マージ状況によらず削除する。対象は worktree、ルートブランチ、所属するスタックブランチであり、`--remote <名前>` がある場合は指定したリモートの同名ブランチを含む。リモート名の省略時は通信もリモート変更も行わない。

### 計画と確認

計画には、登録済み worktree とセッション識別情報、所属ブランチと OID、HEAD、未コミット・未追跡・無視対象ファイルの状態を含める。リモートを指定した場合は、そのリモートの単一の push 先 URL と同名ブランチの OID を取得する。upstream の別名や fetch 先から削除先を推測せず、push 先に存在しないブランチは対象から除く。

メイン・実行中・管理範囲外・未管理・ロック中の worktree、設定のベースブランチ、別 worktree で使用中の所属ブランチを含むセッションは拒否する。対象 worktree の現在のブランチもセッションに属する必要がある。

計画の worktree・ローカルブランチ・リモート名と対象参照・ファイル状態を表示する。`--dry-run` は計画表示で終了する。実行時はファイルの破棄に `--force`、確認省略に `--yes` をそれぞれ要求し、通常の確認は「いいえ」を初期値とする。承認後に計画を再取得し、対象・参照・状態に差があれば変更を始めず停止する。

### 実行と失敗時の状態

削除処理は次の順序で行い、各段階の成功を確認してから次へ進む。

| 段階 | 処理 | 失敗時の状態 |
| --- | --- | --- |
| リモート参照 | 対象がある場合、計画の OID を指定した lease と atomic push で一括削除 | ローカルは保持。通信障害ではリモートの結果が不明な場合がある |
| worktree | `--force` の指定に応じて削除 | ローカルブランチは保持。完了したリモート削除は戻さない |
| ローカル参照 | 計画の OID と一致する所属ブランチをトランザクションで一括削除 | worktree は削除済み、ブランチは保持 |
| ブランチ設定 | 削除したブランチのローカル設定を除去 | worktree と参照は削除済み、未処理の設定は残る |

リモートとローカルをまたぐトランザクションは設けない。失敗時は完了した範囲と残った対象を報告し、全段階が成功した場合だけ完了を表示する。push 先 URL は認証情報を含み得るため表示せず、リモート照会・削除の外部診断もそのまま出力しない。
