...
discard は指定された一つの wts セッションを、マージ状況によらず破棄する。呼び出し元プロジェクトの管理範囲、登録された worktree、セッション記録から対象を決め、所属するルート・スタックブランチを表示して確認する。メイン・実行中・管理範囲外・未管理・ロック中の worktree、ベースブランチや別 worktree で使用中の所属ブランチは保護する。未コミット・未追跡・無視されたファイルを削除するには `--force` が必要であり、確認省略の `--yes` とは独立する。dry-run はファイルの変更状態を含む計画を表示するだけである。確認後に対象とブランチを再検査し、worktree の削除が成功してから所属ブランチを削除する。途中で失敗した場合は失敗を報告し、完了とは扱わない。リモートへの通信と変更は行わない。
