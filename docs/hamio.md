...
| [`src/ui.ts`](../src/ui.ts) | メッセージ・項目一覧・結果を表示 block に変換し、API 上限に従って分割 |
...
| 一度の表示 | 32 blocks。メッセージを複数の render 要求に分割 |
| `key-value` | 1 block に 200 項目。項目一覧を複数の要求に分割 |
| 単発 JSON | 256 KiB。各要求は hamio の入力検証に従う |
