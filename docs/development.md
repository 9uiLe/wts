...
各 HTML は全ケースの画面を内包し、上部の「全件」「変更のみ」でページ内表示を切り替えます。検索とコマンド絞り込みを使って確認できます。変更のないケースは出力を1つだけ表示し、変更があるケースは「変更前」「変更後」を並べます。追加・削除されたケースは該当する出力だけを表示します。`index.html` は全件、`changes.html` は変更のみを初期表示します。サーバーや外部ファイルの読み込みを必要とせず、HTML 単体をブラウザーで開いて共有できます。JSON と Markdown は開発・調査用のファイルとして保存し、レビュー画面には表示しません。CI の生成物は Actions の `terminal-ui-review` アーティファクトから取得できます。
