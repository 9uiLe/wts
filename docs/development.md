...
各 HTML は全ケースの画面を内包し、上部の「全件」「変更のみ」でページ内表示を切り替えます。検索とコマンド絞り込みを使い、基準画面と収録画面を比較できます。`index.html` は全件、`changes.html` は変更のみを初期表示します。サーバーや外部ファイルの読み込みを必要とせず、HTML 単体をブラウザーで開いて共有できます。JSON と Markdown は開発・調査用のファイルとして保存し、レビュー画面には表示しません。CI の生成物は Actions の `terminal-ui-review` アーティファクトから取得できます。
