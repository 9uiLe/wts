...
各 HTML は全ケース、収録データ、比較結果、収録範囲を内包し、上部の操作でページ内表示を切り替えます。`index.html` は全件、`changes.html` は変更のみを初期表示します。サーバーや外部ファイルの読み込みを必要とせず、HTML 単体をブラウザーで開いて共有できます。JSON と Markdown はデータを直接扱うための出力です。CI の生成物は Actions の `terminal-ui-review` アーティファクトから取得できます。
