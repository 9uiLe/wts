...
| --- | --- |
| start・stack | 設定・名前・対象の検査と、名前の解決に必要な命名スクリプト。start で明示した名前に対応するスクリプトは実行しない |
...
命名スクリプトの通信・副作用を把握してから実行してください。スクリプトを動かさず設定だけを検査するには `config check` を使います。dry-run はベース参照を更新せず、名前や対象を予約・固定しません。本実行では再解決・再検査するため、生成する名前や対象の状態は変わる場合があります。

## セッション開始

独立した作業ごとに、新規ブランチと一つの worktree を持つセッションを作ります。確定した名前で並列作業を始めるには、作業ごとに異なるブランチ名・ディレクトリ名を指定します。

```sh
wts --format json start --branch feature/auth --worktree auth --base-branch origin/main
wts --format json start --branch fix/search --worktree search-fix --base-branch origin/main
```

ベースは対象プロジェクトに合わせ、設定済みならオプションを省略できます。ベースが `origin/` で始まれば fetch し、失敗して手元の参照を使う場合は警告します。作成後は各出力の `Path` を、その作業を担当する端末・エージェントの作業ディレクトリにします。呼び出し元のディレクトリは変わりません。

`--branch`・`--worktree` はそれぞれ「明示した名前→対応する命名スクリプト→既定名」の順に解決します。既定のブランチ名は日本時間の `YYYYMMDD-<UUID>`、worktree 名は確定したブランチ名の `/` を `-` に置き換えたものです。名前を明示した側のスクリプトは dry-run でも実行しません。worktree 名は管理範囲内の単一ディレクトリ名で、絶対パス、`/`、`\`、空白だけの名前、`.`、`..`、大文字小文字を問わない `.git`、改行、NUL は使えません。

未指定の名前をスクリプトで生成する場合は、非対話では `--task '作業内容'` を渡します。実行するスクリプトがなければ `--task` は不要です。入力形式と失敗時の扱いは [命名スクリプト](#命名スクリプト)を参照してください。

どの命名方法でも設定全体を検査し、設定したスクリプトの存在・実行権限を要求します。ブランチ名は Git の規則で検査し、既存ブランチ・存在する作成先・登録済み worktree のパスとの衝突を拒否します。
...
start は worktree を作成してセッションを記録してから、管理外ファイルをコピーします。作成後の失敗は自動で元に戻りません。表示された Path・ブランチと、worktree・ブランチ・セッション記録の作成済み範囲を確認し、start を無条件に再実行しないでください。
...
start は明示されていない名前に対応するスクリプトだけを実行し、stack は `naming.branch` で `<root>-pr<n>-` に続く接尾辞を生成します。stack にブランチ命名設定がなければ、日本時間の `YYYYMMDD-<UUID>` を接尾辞に使います。worktree 命名設定は stack では使いません。

`--task` は実行するスクリプトへ渡す作業内容で、名前の直接指定ではありません。実行するスクリプトがある非対話操作には `--task` が必要で、`--task ''` でもスクリプトは実行します。スクリプトを実行しない場合は作業内容の対話を行いません。
