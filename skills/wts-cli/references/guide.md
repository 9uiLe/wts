...
計画に表示される worktree・全ブランチ・ファイル状態が依頼された破棄対象と一致することを確認します。dry-run は削除対象を固定せず、本実行で再検査します。単一ブランチだけの削除には使いません。リモートブランチは残り、ネットワーク通信や gh 認証は不要です。
...
メインチェックアウト、実行中・管理範囲外・未管理・ロックされた worktree、ベースブランチや別の worktree で使用中の所属ブランチを含むセッションは拒否します。worktree の削除失敗時はブランチを削除しません。部分的な失敗は報告を読み、残った対象を確認してください。エラーを回避する目的で無条件に Git の強制削除へ切り替えないでください。
...
| discard | ローカルのセッション・ブランチ・ファイル状態の検査 |
