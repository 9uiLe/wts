...
`discard` は一つの wts セッションの worktree とルート・スタックブランチを、マージ状況によらず削除します。メインチェックアウトなど対象の外から、worktree のルートパスを指定します。

```sh
wts discard /path/to/session --dry-run
```

計画に表示される worktree・全ブランチ・ファイル状態が依頼された破棄対象と一致することを確認します。dry-run は削除対象を固定せず、本実行で再検査します。単一ブランチだけの削除には使いません。既定ではリモートブランチを残し、ネットワーク通信は行いません。

```sh
wts discard /path/to/session --yes
```

`--yes` は対話確認を省略するだけです。未コミット・未追跡・Git で無視されたファイルがある場合は拒否します。それらの破棄まで依頼されているときに限り `--force` を併用します。非対話環境では `--yes` が必要です。

メインチェックアウト、実行中・管理範囲外・未管理・ロックされた worktree、ベースブランチや別の worktree で使用中の所属ブランチを含むセッションは拒否します。worktree の削除失敗時はローカルブランチを削除しません。部分的な失敗は報告を読み、残った対象を確認してください。エラーを回避する目的で無条件に Git の強制削除へ切り替えないでください。

リモートの削除も依頼されている場合は、登録済みのリモート名を `--remote` で指定します。
...
指定したリモートの push 先にある、所属ブランチと同名のブランチだけが対象です。upstream の別名は対象外で、存在しないブランチはスキップします。push 先が複数のリモートは拒否します。Git の push 認証とサーバーの atomic push 対応が必要です。確認時の OID を条件としてリモートを一括削除し、成功後にローカルを削除します。リモート操作の失敗時はローカルを残します。通信障害ではリモートの結果が不明な場合があるため、再実行前に残存ブランチを確認します。ローカル処理だけ失敗した場合も、リモートの削除は元に戻りません。
